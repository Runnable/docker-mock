...
module.exports = {
  ...
  createContainer: function (containers) {
    return function (req, res, next) {
      ...
      var data = {
        ...
        'NetworkSettings': {
          'Ports': {
            '80/tcp':[{ HostPort: newNumber() }],
            '15000/tcp':[{ HostPort: newNumber() }]
          }
        }
      };
  ...
  startContainer: function (containers) {
    return function (req, res, next) {
      ...
      containers[Id].NetworkSettings = NetworkSettings;
      req.data.container.NetworkSettings = NetworkSettings;
      next();
    };
  },
  stopContainer: function (containers) {
    return function (req, res, next) {
      ...
      containers[Id].State = State;
      req.data.container.State = State;
      delete containers[Id].NetworkSettings;
      delete req.data.container.NetworkSettings;
      next();
    };
  },
