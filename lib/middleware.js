...
module.exports = {
    ...
    return function (req, res, next) {
      ...
      req.pipe(tar.Parse()).
        ...
        on('end', function () {
          ...
          tags[name] = id;
          var now = 1411762135;
          imageHistory[id] = dockerfile.split('\n').reverse().map(function (line, index) {
            return {
              Id: utils.randomId(),
  ...
  findContainer: function (containers) {
    return function (req, res, next) {
      ...
      req.data.container = _.find(containers, function (container, id) {
        return id.indexOf(containerId) === 0 || container.name === containerId;
      });
  ...
  respondImageHistory: function (code) {
    return function (req, res, next) {
      if (!req.data || !req.data.image) return res.sendStatus(404); // we didn't find the image
      if (!imageHistory[req.data.image.id]) return res.sendStatus(500); // we don't have a history (we should)
      res.status(code || 200).json(imageHistory[req.data.image.id]);
    };
  },
  ...
  respondContainer: function (code, pick) {
    return function (req, res, next) {
      ...
      if (code && code === 204) res.sendStatus(204);
      else res.status(code || 200).send(req.data.container);
    };
  },
  respondContainers: function (containers) {
    return function (req, res, next) {
      res.status(200).json(_.map(containers, function (container, Id) {
        return {
          'Id': container.Id,
  ...
  notYetImplemented: function (req, res, next) {
    res.status(501).send('endpoint not yet implemented: ' + req.method + ' ' + req.path);
  }
};
