...
module.exports = {
  ...
  createImage: function (images, tags) {
    return function (req, res, next) {
      ...
      if(!from) {
        return res.json(500, '{"status":"Downloading from http://"}\n{"errorDetail":{"message":"Get http://: http: no Host in request URL"},"error":"Get http://: http: no Host in request URL"}');
      }

      images[id] = { id: id };
      tags[from + ":" + (req.query.tag || "latest")] = id;
      emitter.emit({status: "Pulling repository" + from});
      for (var i = 0; i < 100; i++) {
        emitter.emit({status: "Pulling", "progress":"1 B/ 100 B", "progressDetail":{"current":i, "total":100}});
      }
