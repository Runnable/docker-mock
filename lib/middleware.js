...
var middlewares = module.exports = {
  ...
  createContainer: function (req, res, next) {
    ...
    // special case: image-builder containers create images
    var Image = keypath.get(req, 'query.Image') || '';
    var Envs = keypath.get(req, 'query.Env') || [];
    if (~Image.indexOf('image-builder')) {
      var tag;
      var imageId = utils.randomId();
      Envs.some(function (item) {
        if (item.split('=')[0] === 'RUNNABLE_DOCKERTAG') {
          tag = item.split('=')[1];
          return true;
        }
        return false;
  ...
  pushRepo: function (req, res) {
    var repoSplit = req.params.repository.split(':');
    var repo = repoSplit[0];
    var tag = repoSplit[1] || req.query.tag || 'latest';
    repo = repo + ':' + tag;
    var name = [
      req.params.registry,
      req.params.namespace,
